...
def plot_graph_interactive(G, outDir, clusters, threshold=0.5, normalization=lambda x: x, color='colorful', period='full', mode='full', annotators = [], summary_statistic=np.median, s=2, node_size=10, node_label='cluster', edge_label_style='weight', edge_width=2, k = 0.8, is_spring=False, seed=0, pos={}):
    """
    ...
    for n, cluster in enumerate(clusters):
        for node in cluster:
            ...
            if node_label=='None':
                label = ' '  
            G_int.add_node(n_id = node,label = label,shape='circle',size=node_size,physics=False,x=x,y=y,color=colors[n],title=text) 
...
def plot_graph_static(G, outDir, clusters, threshold=0.5, normalization=lambda x: x, color='colorful', period='full', mode='full', annotators = [], summary_statistic=np.median, s=2, node_size=80, edge_width=1, k=0.8, is_spring=False, seed=0, is_edge_labels=False, edge_label_style='weight', dpi=300, font_size_nodes=11, font_size_edges=11, node_label_style=None, pos={}):
    """
    ...
    for n, cluster in enumerate(clusters):
        ...
        for node in cluster:
            label = ''
            if not node in nodes: # skip nodes not in time period
                nx.draw_networkx_nodes(G,pos,node_size=node_size,alpha=0.0,node_color=colors[n],nodelist=[node])
                continue
            ...
            nx.draw_networkx_nodes(G,pos,node_size=node_size,alpha=1.0,node_color=colors[n],nodelist=[node])
